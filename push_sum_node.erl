    ...
    receive
        {NeighborPid, FinishCount} ->
            ...
            if (NeighborAlive) ->
                ...
                % NewSendCount = 0;
            true ->
                if (FinishCount == NodeCount) ->
                    send_neighbor(ParentIndex, NodeCount, Sum, Weight, Topology, ParentPid, Master, NodeCount);
                true ->
                    % if (SendCount == 10) ->
                    %     if (Algorithm == 4) ->
                    %         Algorithm = 1,
                    io:fwrite("~p trying to send ", [ParentIndex]),
                    send_neighbor(ParentIndex, NodeCount, Sum, Weight, Topology, ParentPid, Master, SendCount)
                end
            end,
    ParentPid
